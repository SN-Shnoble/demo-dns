                ...
                <el-table-column :label="$t('admin.menuConfig.name') || '菜单名称'" min-width="250">
                    <template #default="{ row }">
                        <span style="display:flex;align-items:center;gap:8px;">
                            <el-icon style="font-size:18px"><component :is="row.icon" /></el-icon>
        ...
        <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
            <el-form :model="editingMenu" label-position="left" label-width="120px">
                <el-form-item :label="$t('admin.menuConfig.name') || '菜单名称'">
                    <el-input v-model="editingMenu.labelKey" />
                </el-form-item>
...
const iconOptions = [
    'DataAnalysis', 'Monitor', 'Upload', 'Aim', 'Collection', 'Document',
    'Message', 'User', 'Connection', 'Coin', 'Wallet', 'Setting', 'Tools', 'UserFilled'
]
...
const defaultMainMenuItems = [
    ...
    { id: 'publishes', labelKey: 'nav.publishes', path: '/admin/publishes', icon: 'Upload', visible: true, sort: 3 },
    { id: 'geo-dns', labelKey: 'nav.geoDns', path: '/admin/geo-dns', icon: 'Aim', visible: true, sort: 4 },
    { id: 'rules', labelKey: 'nav.ruleLibrary', path: '/admin/rules', icon: 'Collection', visible: true, sort: 5 },
    { id: 'query-logs', labelKey: 'admin.queryLogs', path: '/admin/query-logs', icon: 'Document', visible: true, sort: 6 },
    { id: 'alerts', labelKey: 'admin.alerts', path: '/admin/alerts', icon: 'Message', visible: true, sort: 7 },
    { id: 'users', labelKey: 'admin.users', path: '/admin/users', icon: 'User', visible: true, sort: 8 },
    { id: 'devices', labelKey: 'admin.devices', path: '/admin/devices', icon: 'Connection', visible: true, sort: 9 },
    { id: 'billing', labelKey: 'admin.billing.title', path: '/admin/billing', icon: 'Coin', visible: true, sort: 10 },
    { id: 'finance', labelKey: 'admin.finance.menu', path: 'finance', icon: 'Wallet', visible: true, sort: 11 },
    { id: 'system-config', labelKey: 'nav.systemConfig', path: '/admin/system-config', icon: 'Tools', visible: true, sort: 12 },
    { id: 'basic-config', labelKey: 'admin.basicConfig.title', path: '/admin/basic-config', icon: 'Setting', visible: true, sort: 13 },
    { id: 'audit-logs', labelKey: 'nav.auditLogs', path: '/admin/audit-logs', icon: 'Document', visible: true, sort: 14 },
    { id: 'rbac', labelKey: 'admin.rbac.title', path: '/admin/rbac', icon: 'UserFilled', visible: true, sort: 15 },
]
...
onMounted(async () => {
    try {
        const { data } = await client.get('/admin/menu-config').catch(() => ({
            data: { data: null },
        }))

        if (data.data) {
            if (data.data.mainMenu) {
                mainMenuItems.value = data.data.mainMenu
            }
            if (data.data.subMenu) {
                subMenuItems.value = data.data.subMenu
            }
        }
    } catch {}
})
